...
import top.wade.enums.CommentOpenStateEnum;
...
public class CommentController {
     ...
     */
    @GetMapping("/comments")
    public Result comments(@RequestParam Integer page, @RequestParam(defaultValue = "") Long blogId, @RequestParam(defaultValue = "1") Integer pageNum,
                           @RequestParam(defaultValue = "10") Integer pageSize, @RequestHeader(value = "Authorization", defaultValue = "") String jwt) {
    ...
    @PostMapping("/comment")
    public Result postComment(@RequestBody Comment comment, HttpServletRequest request, @RequestHeader(value = "Authorization", defaultValue = "") String jwt){
        //评论内容合法性校验
        if (StringUtils.isEmpty(comment.getContent()) || comment.getContent().length() > 250 || comment.getPage() == null || comment.getParentCommentId() == null) {
            return Result.error("参数有误");
        }
        //是否是访客的评论
        boolean isVisitorComment = false;
        //父评论
        top.wade.entity.Comment parentComment = null;
            ...
            case PASSWORD:
                ...
                if (JwtUtils.judgeTokenIsExist(jwt)) {
                    ...
                    if (subject.startsWith(JwtConstants.ADMIN_PREFIX)) {
                        ...
                        //普通访客经文章密码验证后携带Token
                        //对访客的评论昵称、邮箱合法性校验
                        if (StringUtils.isEmpty(comment.getNickname(), comment.getEmail()) || StringUtils.isEmpty() ||comment.getNickname().length() > 15) {
                            return Result.error("参数有误");
                        }
